...
struct {
  ...
  char *filename;
                   ...
                   { .input = 32, .output = 32, .lr = 0.00004, .filename = "save2.ckpt" },
                   { .input = 32, .output = 64, .lr = 0.00004, .filename = "save3.ckpt" },
                   { .input = 64, .output = 256, .lr = 0.00004, .filename = "save4.ckpt" },
                   { .input = 256, .output = 128, .lr = 0.00004, .filename = "save5.ckpt" },
                   { .input = 256, .output = 10, .lr = 0.00004, .filename = "save6.ckpt" } };
...
int main(int argc, char **argv) {
 ...
 for (unsigned int i=0; i<CASES; i++) {
   ...
   linearInfo(l_test);
   if (verifyWeights(l_test) < 0) printf("Linear Layer Corruption.\n");
   linearSaveCheckpoint(l_test, test_cases[i].filename);
   linearFree(l_test);
   l_test = linearLoadCheckpoint(test_cases[i].filename);
   linearInfo(l_test);
   if (verifyWeights(l_test) < 0) printf("Loaded Layer Corruption.\n");
   linearFree(l_test);
 }
}
